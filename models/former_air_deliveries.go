...
type FormerAirDeliveries struct {
	...
	ArrivalDate               time.Time `xorm:"comment('到达日期') DATETIME"`
	CourierCompanyId          int       `xorm:"comment('快递公司') INT(11)"`
	CourierNumber             string    `xorm:"comment('快递单号') VARCHAR(64)"`
	FlightCompanyId           int       `xorm:"comment('航空公司') INT(11)"`
	FlightNumber              string    `xorm:"comment('航班') VARCHAR(128)"`
	WarehousingDate           time.Time `xorm:"comment('入仓日期') DATETIME"`
	...
	TransshipmentFlight       string    `xorm:"comment('转运航班') VARCHAR(128)"`
	TransshipmentEtd          time.Time `xorm:"comment('转运ETD') DATETIME"`
