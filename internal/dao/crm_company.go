...
func (c CrmCompany) UpdateByMap(id uint, attr map[string]interface{}) error {
	...
	err := global.DataEngine.Model(&company).Updates(attr).Error
	if err != nil {
		return err
	}
	...
	if companyType == 3 {
		_ = global.DataEngine.Model(&company).Association("Roles").Append(models.Role{
...
func (c CrmCompany) Update(id uint, company models.CrmCompany) (models.CrmCompany, error) {
	...
	if err := global.DataEngine.Model(&models.CrmCompany{ID: id}).Association("Roles").Replace(company.Roles); err != nil {
		return record, err
	}
	err := global.DataEngine.Model(&models.CrmCompany{ID: id}).Set("gorm:association_autocreate", false).Updates(company).Error
	return company, err
}
...
func (c CrmCompany) Find(per, page int, filter map[string]interface{}, selectKeys []string,
	...
	sqlCon := global.DataEngine.Model(&models.CrmCompany{})
	if isTotal {
		if total, err = c.Count(global.DataEngine.Model(&models.CrmCompany{}), filter); err != nil {
			return
		}
	}
	if len(selectKeys) == 0 {
...
func (c CrmCompany) Create(company models.CrmCompany) (models.CrmCompany, error) {
	err := global.DataEngine.Model(&models.CrmCompany{}).Create(&company).Error
	return company, err
}

func (c CrmCompany) First(id uint, preload ...string) (company models.CrmCompany, err error) {
	sqlConn := global.DataEngine.Model(&models.CrmCompany{})
